...
// استيراد النافذة المنبثقة الجديدة (تأكد من مسار الملف الصحيح لديك)
import StartProjectModal from "@/components/start-project-modal"

export default function Navbar() {
  const [open, setOpen] = useState(false)
  // State خاص بالتحكم في نافذة نموذج الطلب
    ...
    <header className="fixed left-0 top-0 z-50 w-full border-b border-white/10 bg-[#020817]/80 backdrop-blur-xl">
      <div className="mx-auto flex h-20 max-w-7xl items-center justify-between px-6">
        ...
        <nav className="hidden items-center gap-8 text-sm text-gray-300 md:flex">
          ...
          <a href="#why-operix" className="transition hover:text-yellow-400">
            why operix249
          </a>
        </nav>
        ...
        <div className="flex items-center gap-4">
          <div className="hidden lg:flex items-center gap-3 rounded-full border border-white/10 bg-white/5 px-4 py-2 text-sm text-gray-300 backdrop-blur-xl">
            <Languages size={16} />
        ...
        {open && (
          <>
            ...
            >
              ...
              {/* language */}
              <div className="hidden md:flex items-center gap-3 rounded-full border border-white/10 bg-white/5 px-4 py-2 text-sm text-gray-300 backdrop-blur-xl mt-6">
                <Languages size={16} />
                <button className="text-yellow-400">العربية</button>
                <span>|</span>
                <button>EN</button>
              </div>

              {/* CTA - زر الجوال لفتح النافذة وإغلاق القائمة */}
              <Button 
                onClick={() => {
                  setIsModalOpen(true);
                  setOpen(false); // إغلاق القائمة الجانبية لتجربة أفضل
                }}
                className="mt-auto bg-yellow-500 text-black hover:bg-yellow-400"
              >
                Start Project
              </Button>
            </motion.div>
      ...
      {/* استدعاء المكون الجديد وتمرير حالة الفتح والإغلاق */}
      <StartProjectModal 
        open={isModalOpen} 
