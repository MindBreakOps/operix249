"use client" // تمت الإضافة: ضروري لاستخدام useState

import React, { useState } from 'react';
import Image from 'next/image';
import { Play, Users, ClipboardCheck, Award, Code2, ArrowUpLeft } from 'lucide-react';
// تمت الإضافة: استيراد مكون النافذة المنبثقة (تأكد من صحة المسار حسب هيكل مشروعك)
import StartProjectModal from '../components/start-project-modal';

export default function HeroSection() {
  // تمت الإضافة: حالة التحكم في ظهور النافذة
  const [isModalOpen, setIsModalOpen] = useState(false);

  return (
    <>
      {/* تمت إضافة id="home" هنا بدلاً من وضعه كعنصر فارغ في الأسفل */}
      <section id="home" className="relative min-h-screen flex flex-col justify-between overflow-hidden bg-[#03060f] pt-20">
          ...
          <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 items-center mt-auto mb-auto">
            ...
            <div className="lg:col-span-7 flex flex-col text-right" dir="rtl">
              ...
              <div className="flex flex-wrap items-center gap-4">
                {/* تمت الإضافة: حدث onClick لفتح النافذة */}
                <button 
                  onClick={() => setIsModalOpen(true)}
                  className="flex items-center justify-center gap-2 bg-amber-400 text-black font-bold px-8 py-4 rounded-xl hover:bg-amber-500 transition-all duration-300 group"
                >
      ...
      {/* تمت الإضافة: مكون النافذة المنبثقة يوضع خارج الـ section الرئيسي */}
      <StartProjectModal 
        open={isModalOpen} 
